...
void solve(){
    ...
    if (n%2 == 0){
        rep(0,n){
            cent_pascals[i] = (factorial[(2*i)]%m)*fast_pow(fast_pow(factorial[i],2,m),m-2,m)%m;
            println(cent_pascals[i])
        }
    }
    else{
         rep(0,n){
            cent_pascals[i] = (((factorial[(2*(i+1))])%m*(fast_pow(fast_pow(factorial[(i+1)],2,m),m-2,m))%m*(fast_pow(2,m-2,m))%m))%m;
        }
        cent_pascals[0] = 0;
    }
    ...
    println("");
    rep2(n){
        if (i >= ((n+1)/2)) counter = (counter + (a[i]*(-cent_pascals[n-(i+1)]))%m)%m;
        else counter = (counter +(a[i]%m*(cent_pascals[i]%m)%m)%m)%m;
